...
#     attr_reader :parser, :nodes_database_path, :pbf_file, :progress_bar 
    
#     @@kc_batch_size = 100000
#     cattr_reader :kc_batch_size

#     def initialize(pbf_file, nodes_database_path = "/tmp/osm_pbf.kch")
#       @pbf_file = pbf_file
...
#     def nodes_database
#       @nodes_database ||= DB::new
#     end
    
#     def open_nodes_database(path)
#       nodes_database.open(path + "#apow=8#opts=l#bnum=2000000#msiz=50000000", DB::OWRITER | DB::OCREATE | DB::OTRUNCATE)
#     end
...
#     def import      
#       open_nodes_database(nodes_database_path)
...
#     def backup_nodes_kc
#       p "Begin to backup nodes in KyotoCabinet nodes_database in #{nodes_database_path}"
#       start = Time.now
...
#       end
#       p "Finish to backup #{nodes_counter} nodes in KyotoCabinet nodes_database in #{(Time.now - start)} seconds"
#     end  
...
#               tags = selected_tags(way[:tags], way_selected_tags_keys)             
...
#       nodes_database.accept_bulk(node_ids) { |key, value|
#         node = Marshal.load(value)
#         node.add_way(way_id)
