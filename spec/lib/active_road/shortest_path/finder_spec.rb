...
describe ActiveRoad::ShortestPath::Finder do
  ...
  let(:first_road) { create(:physical_road) }
  let(:second_road) { create(:physical_road) }
  let(:third_road) { create(:physical_road) }

  let(:source) { first_road.geometry.first.endpoint(90, 50) }
  let(:destination) { third_road.geometry.last.endpoint(0, 50) }
  
  let!(:first_junction) { create(:junction, :physical_roads => [first_road, second_road])}
  let!(:second_junction) { create(:junction, :physical_roads => [second_road, third_road])}
