...
describe ActiveRoad::ShortestPath::Finder do
  ...
  let(:first_road) { create(:physical_road, :objectid => "1",:geometry => line_string("0 0, 1 1"), :tags => {:speed => "100"}) }
  let(:second_road) { create(:physical_road, :objectid => "2", :geometry => line_string("1 1, 2 2")) }
  let(:third_road) { create(:physical_road, :objectid => "3", :geometry => line_string("2 2, 3 3")) }

  let(:second_road_bis) { create(:physical_road, :objectid => "2bis", :geometry => line_string("1 1, 3 1, 3 3"), :tags => {:speed => "100"}) }
