...
describe ActiveRoad::OsmPbfImporter2 do
  let(:pbf_file) { File.expand_path("../../../fixtures/test.osm.pbf", __FILE__) }

  subject { ActiveRoad::OsmPbfImporter2.new( pbf_file, "/tmp/osm_pbf_test_leveldb" ) }
  ...
  describe "#update_node_with_ways" do
    ...
    before :each do 
      subject.database.put("1", Marshal.dump(ActiveRoad::OsmPbfImporter2::Node.new("1", 2.0, 2.0)) )
      subject.database.put("2", Marshal.dump(ActiveRoad::OsmPbfImporter2::Node.new("2", 2.0, 2.0)) )
      subject.database.put("3", Marshal.dump(ActiveRoad::OsmPbfImporter2::Node.new("3", 2.0, 2.0)) )
    end
  ...
  describe "#iterate_nodes" do
    ...
    before :each do 
      subject.database.put("1", Marshal.dump(ActiveRoad::OsmPbfImporter2::Node.new("1", 2.0, 2.0, ["1", "2"])) )
      subject.database.put("2", Marshal.dump(ActiveRoad::OsmPbfImporter2::Node.new("2", 2.0, 2.0, ["1", "3"])) )
    end
  ...
  describe "#way_geometry" do
    ...
    before :each do 
      subject.database.put("1", Marshal.dump(ActiveRoad::OsmPbfImporter2::Node.new("1", 0.0, 0.0)) )
      subject.database.put("2", Marshal.dump(ActiveRoad::OsmPbfImporter2::Node.new("2", 1.0, 1.0)) )
      subject.database.put("3", Marshal.dump(ActiveRoad::OsmPbfImporter2::Node.new("3", 2.0, 2.0)) )
    end
  ...
  describe ActiveRoad::OsmPbfImporter2::Node do
    
    let(:subject) { ActiveRoad::OsmPbfImporter2::Node.new("122323131", 34.2, 12.5) } 

    it "should return a node object when unmarhalling a dump of node object" do
      data = Marshal.dump(subject)
      object = Marshal.load(data)
      object.should be_an_instance_of(ActiveRoad::OsmPbfImporter2::Node)
