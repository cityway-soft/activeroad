...
namespace :active_road do
  ...
  namespace :import do
    ...
    task :filter_osm_pbf_data, [:file, :poly_file] => [:environment] do |task, args|
      ...
      sh %Q{osmosis --read-pbf file="#{args.file}" --bounding-polygon file="#{args.poly_file}" completeWays=yes completeRelations=yes --tf accept-ways highway=* addr:interpolation=* addr:housenumber=* --tf reject-ways construction=yes,1,true --write-pbf file="ontario_mtx.osm.pbf"}
    end
